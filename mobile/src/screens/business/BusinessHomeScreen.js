...
function VenueCard({ venue, sub, onDelete, navigation, openReservations = false, highlightReservationId = null, highlightDate = null, openOrders = false, highlightActivityId = null, openClubs = false }) {
    ...
    const handleGlobalSlotType = async (type) => {
        if (sortedCourts.every(c => courtSlotTypes[c.id] === type)) return;
        setSavingSlot(true);
        try {
            await Promise.all(sortedCourts.map(c =>
                api.patch(`/venues/${venue.id}/courts/${c.id}/settings`, { slotType: type })
            ));
            const upd = {};
            sortedCourts.forEach(c => { upd[c.id] = type; });
            setCourtSlotTypes(prev => ({ ...prev, ...upd }));
            setGlobalSlotType(type);
        } catch (e) { Alert.alert('Hata', e?.response?.data?.message || 'Kaydedilemedi'); }
        finally { setSavingSlot(false); }
    };

    const handleGlobalSurface = async (surface) => {
        const isAllSame = sortedCourts.every(c => courtSurfaces[c.id] === surface);
        const next = isAllSame ? null : surface;
        try {
            await Promise.all(sortedCourts.map(c =>
                api.patch(`/venues/${venue.id}/courts/${c.id}/settings`, { surface: next })
            ));
            const upd = {};
            sortedCourts.forEach(c => { upd[c.id] = next; });
    ...
    const handleSaveContactLinks = async () => {
        ...
        try {
            ...
            await api.patch(`/venues/${venue.id}/settings`, { lat, lng, contactLinks: clean });
            Alert.alert('✅ Kaydedildi', 'Konum ve iletişim butonları güncellendi.');
        } catch (e) { Alert.alert('Hata', e?.response?.data?.message || 'Kaydedilemedi'); }
    ...
    const handleSaveReservationOpenDaysBefore = async (daysBefore) => {
        setSavingReservationWindow(true);
        try {
            await api.patch(`/venues/${venue.id}/settings`, { reservationOpenDaysBefore: daysBefore });
            setLocalReservationOpenDaysBefore(daysBefore);
        } catch (e) { Alert.alert('Hata', e?.response?.data?.message || 'Kaydedilemedi'); }
        finally { setSavingReservationWindow(false); }
    };

    const handleSaveReservationOpenTime = async () => {
        const t = localReservationOpenTime.trim();
        if (t && !/^\d{2}:\d{2}$/.test(t)) { Alert.alert('Hata', 'Geçerli saat girin (HH:MM, ör: 09:00)'); return; }
        setSavingReservationWindow(true);
        try {
            await api.patch(`/venues/${venue.id}/settings`, { reservationOpenTime: t || null });
        } catch (e) { Alert.alert('Hata', e?.response?.data?.message || 'Kaydedilemedi'); }
        finally { setSavingReservationWindow(false); }
    };
    ...
    useEffect(() => {
        if (openClubs) setManageOpen(true);
    }, [openClubs]);
    ...
    const statusCompact = venue.status === 'APPROVED';

    return (
        <View style={vc.card}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 2 }}>
                <Text style={{ color: bMeta.color, fontSize: 11, fontWeight: '900', letterSpacing: 0.5 }} numberOfLines={1}>
                    {branchTitle}
                </Text>
                <Text style={{ color: '#4b5563', fontSize: 11 }}>·</Text>
                <Text style={{ color: '#fff', fontSize: 13, fontWeight: '900', flexShrink: 1 }} numberOfLines={1}>
                    {venue.name}
                </Text>
                {placeLine ? (
                    <>
                        <Text style={{ color: '#4b5563', fontSize: 11 }}>·</Text>
                        <Text style={{ color: colors.textMuted, fontSize: 11, flexShrink: 1 }} numberOfLines={1}>
                            {placeLine}
                        </Text>
                    </>
                ) : null}
                {statusCompact ? (
                    <Text style={{ color: '#22c55e', fontSize: 14, fontWeight: '900', marginLeft: 2 }}>✓</Text>
                ) : (
                    <View style={[vc.badge, { backgroundColor: statusColor + '20', borderColor: statusColor + '60', marginLeft: 2 }]}>
                        <Text style={[vc.badgeText, { color: statusColor, fontSize: 9 }]} numberOfLines={1}>{statusLabel}</Text>
                    </View>
                )}
            </View>
            ...
            {isApproved ? (
                <View style={{ flexDirection: 'row', gap: 6, marginTop: 12 }}>
                    ...
                    {isPro ? (
                        <TouchableOpacity
                            onPress={() => openQuickPanel('bills')}
                            style={{ flex: 1, backgroundColor: '#ffffff08', borderRadius: 8, paddingVertical: 9, alignItems: 'center', borderWidth: 1, borderColor: '#ffffff18' }}
                        >
                            <Text style={{ color: '#e5e7eb', fontWeight: '800', fontSize: 10 }} numberOfLines={1}>Adisyon</Text>
                        </TouchableOpacity>
                    ) : null}
                    {isPro ? (
                        <TouchableOpacity
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: isApproved ? 8 : 12, alignItems: 'stretch' }}>
                {isApproved ? (
                    <TouchableOpacity
                        onPress={() => { setActiveTab('settings'); setManageOpen(true); }}
            ...
            <Modal visible={!!quickOpen} animationType="slide" onRequestClose={() => setQuickOpen(null)}>
            ...
            {activeTab === 'settings' && (
                <View style={vc.panel}>
                    ...
                    {/* Çalışma Saatleri + Gece Işıkları — hem "Tüm Kortlar" hem tek kort görünümünde
                        ...
                        şeklinde kullanılıyor — içerik aynı, sadece render pozisyonu değişiyor. */}
                    ...
                    return (
                    <>
                    {/* ── Kort seçici (tüm panel için) — satıra 5 sığacak şekilde, taşan 2. satıra geçer.
                        flexBasis:'18%' + columnGap/rowGap ile 5x(18%+gap) toplamı %100'ü aşmıyor. */}
                    {sortedCourts.length > 1 && (
                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', columnGap: 5, rowGap: 8, marginBottom: 16 }}>
                            <TouchableOpacity onPress={() => { setSelectedCourt(null); setAddingWindow(false); setNewFrom(''); setNewTo(''); }}
                                style={{ flexBasis: '18%', flexGrow: 1, paddingHorizontal: 6, paddingVertical: 5, borderRadius: 7, borderWidth: 1.5,
                                    alignItems: 'center',
                    ...
                    {!selectedCourt ? (
                        /* ── Global ayarlar: tüm kortlara uygulanır ── */
                        <>
                                ...
                                <View style={{ flex: manySurfaces ? 2 : 1 }}>
                                    ...
                                    {globalSurface && (
                                        <Text style={{ color: '#555', fontSize: 11, marginTop: 6 }}>
                                            Seçili: {SURFACE_ICON[globalSurface]} {SURFACE_LABEL[globalSurface]}
                                        </Text>
                                    )}
                                </View>
                                <View style={{ flex: 1 }}>
                            ...
                            <Text style={{ color: '#555', fontSize: 11, marginBottom: 14, lineHeight: 15 }}>
                                Alan türü tüm kortlara uygulanır, bireysel kortlar ayrı özelleştirilebilir.
                            </Text>
                        ...
                        /* ── Kort özel ayarları ── */
                        (() => {
                            const court = sortedCourts.find(c => c.id === selectedCourt);
                            ...
                            return (
                                <View style={{ marginBottom: 28 }}>
                                    ...
                                    <Text style={{ color: '#555', fontSize: 11, marginBottom: 14, lineHeight: 15 }}>
                                        Alan türü varsayılanı: {globalIndoor ? '🏠 Kapalı' : '🌤️ Açık'}. Farklıysa bu kort için özelleştir, tekrar tıklayarak varsayılana dön.
                                    </Text>
                    ...
                    <View style={{ height: 1, backgroundColor: '#ffffff10', marginVertical: 20 }} />
                    <Text style={{ color: '#666', fontSize: 11, fontWeight: '700', marginBottom: 4, letterSpacing: 0.5 }}>
                        REZERVASYON AÇILIŞ SÜRESİ
                    </Text>
                    <Text style={{ color: '#555', fontSize: 11, marginBottom: 10, lineHeight: 16 }}>
                        Bir günün rezervasyonları kaç gün önceden açılsın? (Örn: 3 gün seçilirse, Salı günü rezervasyonları Pazar günü açılır)
                    </Text>
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
                        {[
                            ...
                            const isActive = localReservationOpenDaysBefore === opt.val;
                            return (
                                <TouchableOpacity key={String(opt.val)} disabled={savingReservationWindow}
                                    onPress={() => handleSaveReservationOpenDaysBefore(opt.val)}
                                    style={{
                    ...
                    {localReservationOpenDaysBefore != null && (
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                            <TextInput
                                style={{ flex: 1, backgroundColor: '#ffffff0a', borderRadius: 8, paddingHorizontal: 12,
                                    paddingVertical: 7, color: '#fff', fontSize: 14, borderWidth: 1, borderColor: '#ffffff15' }}
                                placeholder="Açılış saati (ör: 09:00, boş=00:00)"
                                placeholderTextColor="#444"
                                value={localReservationOpenTime}
                                onChangeText={setLocalReservationOpenTime}
                                returnKeyType="done"
                                onSubmitEditing={handleSaveReservationOpenTime}
                            />
                            <TouchableOpacity disabled={savingReservationWindow}
                                onPress={handleSaveReservationOpenTime}
