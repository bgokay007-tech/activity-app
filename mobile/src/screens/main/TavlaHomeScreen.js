        ...
        <TouchableOpacity style={s.eventCard} onPress={() => onOpen(item)} activeOpacity={0.8}>
            ...
            {(item.matchDate || item.matchTime) ? (
                <Text style={s.eventMeta}>
                    📅 {item.matchDate ? fmtDate(new Date(item.matchDate)) : (t.tavlaFlexibleDate || 'Esnek tarih')}{item.matchTime ? ` · ${item.matchTime}` : ''}
                </Text>
            ) : null}
            {!isOwner && (
                <TouchableOpacity
...
export default function TavlaHomeScreen({ navigation }) {
    const t = useT();
    ...
    useEffect(() => {
        ...
        if (socket) socket.emit('tavla:setUsername', myName);
    }, [myName]);
    ...
    useEffect(() => {
        ...
        const offMatched = onSocket('tavla:matched', (data) => {
            ...
            const me = data.players.find(p => p.userId === myId);
            if (!me) return;
            navigatedRef.current = true;
            setSearching(false);
            navigation.navigate('TavlaTable', { tableId: data.tableId });
        });
        const offError = onSocket('tavla:error', (data) => {
            setSearching(false);
    ...
    const startSearch = () => {
        const socket = getSocket();
        if (!socket) return Alert.alert('', t.tavlaNoConnection || 'Bağlantı kurulamadı, tekrar deneyin.');
        setSearching(true);
        setQueuePos(null);
    ...
    const cancelSearch = () => {
        getSocket()?.emit('tavla:cancelFindMatch');
        setSearching(false);
        setQueuePos(null);
    };
    ...
    const loadEvents = useCallback(async () => {
        ...
        try {
            const { data } = await api.get('/rivals', { params: { category: 'GAMES', subCategory: 'tavla' } });
            setEvents(Array.isArray(data) ? data : []);
        } catch (e) {
            Alert.alert(t.error || 'Hata', e?.response?.data?.message || t.tavlaEventsLoadError || 'İlanlar yüklenemedi.');
        } finally {
    ...
    const submitEvent = async () => {
        if (!eventForm.message.trim()) return Alert.alert('', t.tavlaEventMsgRequired || 'Açıklama girin.');
        setCreatingEvent(true);
        try {
            await api.post('/rivals', {
                category: 'GAMES', subCategory: 'tavla',
            ...
            Alert.alert('', e?.response?.data?.message || t.tavlaEventCreateError || 'İlan oluşturulamadı.');
        } finally {
    ...
    const joinEvent = async (item) => {
        try {
            ...
            Alert.alert('', e?.response?.data?.message || t.tavlaJoinError || 'İstek gönderilemedi.');
        }
    };

    const openEvent = (item) => {
        navigation.navigate('SubCategory', { category: 'GAMES', sub: 'tavla', initialTab: 'rivals', highlightRivalId: item.id });
    };
    ...
    return (
        <View style={s.root}>
            ...
            <View style={s.mainTabRow}>
                {[
                    { id: 'play',   label: t.tavlaTabPlay   || '🎲 Tavla Oyna' },
                    { id: 'events', label: t.tavlaTabEvents || '📋 Tavla İlanı' },
                ].map(tb => (
            ...
            {mainTab === 'play' ? (
                <View style={s.playWrap}>
                    ...
                    <Text style={s.playTitle}>{t.tavlaPlayTitle || 'Gerçek Zamanlı Tavla'}</Text>
                    <Text style={s.playDesc}>{t.tavlaPlayDesc || '2 kişilik masaya otomatik eşleşerek uygulama içinde canlı Tavla oyna.'}</Text>

                    {searching ? (
                        <View style={{ alignItems: 'center', marginTop: 30 }}>
                            <ActivityIndicator size="large" color={colors.purple} />
                            <Text style={s.searchingText}>
                                {t.tavlaSearching || 'Rakip aranıyor...'}{queuePos ? ` (${queuePos}/2)` : ''}
                            </Text>
                        ...
                        <>
                            <TouchableOpacity style={s.findBtn} onPress={startSearch} activeOpacity={0.85}>
                                <Text style={s.findBtnText}>{t.tavlaFindMatch || '🔍 Rakip Ara'}</Text>
                            </TouchableOpacity>

                            <Text style={s.orText}>{t.tavlaOr || 'veya'}</Text>

                            <Text style={s.difficultyLabel}>{t.tavlaDifficultyLabel || 'Bot Zorluğu'}</Text>
                            <View style={s.difficultyRow}>
                                {[
                                    { id: 'easy',   label: t.tavlaDifficultyEasy   || 'Kolay' },
                                    { id: 'medium', label: t.tavlaDifficultyMedium || 'Orta' },
                                    { id: 'hard',   label: t.tavlaDifficultyHard   || 'Zor' },
                                ].map(d => (
                            ...
                            <TouchableOpacity style={s.botBtn} onPress={startVsBots} activeOpacity={0.85}>
                                <Text style={s.botBtnText}>{t.tavlaPlayVsBots || '🤖 Botla Oyna'}</Text>
                            </TouchableOpacity>
                        </>
                    )}
                </View>
                ...
                <>
                    <View style={s.smallCreateRow}>
                        <TouchableOpacity style={s.smallCreateBtn} onPress={() => setShowCreateEvent(true)}>
                            <Text style={s.smallCreateBtnText}>{t.tavlaCreateListingBtn || '+ İlan Oluştur'}</Text>
                        </TouchableOpacity>
                    </View>
                    {eventsLoading ? (
                        ...
                        <FlatList
                            ...
                            ListEmptyComponent={eventsLoaded ? <Text style={s.emptyText}>{t.tavlaNoEvents || 'Henüz ilan yok. İlk ilanı siz oluşturun!'}</Text> : null}
                        ...
                        <View style={s.modalOverlay}>
                            <View style={s.modalBox}>
                                <ScrollView showsVerticalScrollIndicator={false}>
                                    <Text style={s.modalTitle}>{t.tavlaCreateListingBtn || '📅 İlan Oluştur'}</Text>

                                    <Text style={s.fieldLabel}>{t.tavlaEventMsgLabel || 'Açıklama *'}</Text>
                                    <TextInput
                                        value={eventForm.message}
                                        onChangeText={v => setEventForm(f => ({ ...f, message: v }))}
                                        placeholder={t.tavlaEventMsgPh || 'Ör: Cumartesi akşamı Tavla oynayacak biri arıyorum.'}
                                        placeholderTextColor={colors.textMuted}
                                        style={[s.modalInput, { height: 80, textAlignVertical: 'top' }]}
                                        multiline
                                    />

                                    <Text style={s.fieldLabel}>{t.tavlaVenueLabel || 'Mekan / Yer'}</Text>
                                    <TextInput
                                        value={eventForm.venueName}
                                        onChangeText={v => setEventForm(f => ({ ...f, venueName: v }))}
                                        placeholder={t.tavlaVenuePh || 'Ör: Kadıköy Kıraathane'}
                                        placeholderTextColor={colors.textMuted}
                                        style={s.modalInput}
                                    />

                                    <Text style={s.fieldLabel}>{t.tavlaCityLabel || 'Şehir'}</Text>
                                    <TextInput
                                        value={eventForm.city}
                                        onChangeText={v => setEventForm(f => ({ ...f, city: v }))}
                                        placeholder={t.tavlaCityPh || 'Ör: İstanbul'}
                                        placeholderTextColor={colors.textMuted}
                                        style={s.modalInput}
                                    />
                                    ...
                                    <View style={{ flexDirection: 'row', gap: 8 }}>
                                        <View style={{ flex: 1 }}>
                                            <Text style={s.fieldLabel}>{t.tavlaDateLabel || 'Tarih'}</Text>
                                            <TouchableOpacity style={s.modalInput} onPress={() => setShowEventDatePicker(true)}>
                                                <Text style={{ color: eventForm.date ? '#fff' : colors.textMuted }}>
                                                    {eventForm.date ? fmtDate(eventForm.date) : (t.tavlaFlexibleDate || 'Esnek tarih')}
                                                </Text>
                                            </TouchableOpacity>
                                        </View>
                                        <View style={{ flex: 1 }}>
                                            <Text style={s.fieldLabel}>{t.tavlaTimeLabel || 'Saat'}</Text>
                                            <TextInput
                                    ...
                                    <TouchableOpacity style={s.submitBtn} onPress={submitEvent} disabled={creatingEvent}>
                                        <Text style={s.submitBtnText}>{creatingEvent ? '…' : (t.tavlaCreateListingBtn || 'İlan Oluştur')}</Text>
                                    </TouchableOpacity>
