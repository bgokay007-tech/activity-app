            ...
            prisma.activityRequest.create({
                ...
                if (rawInvites.length > 0) {
                    const eligible = await prisma.refereeListing.findMany({
                        where: { userId: { in: rawInvites.map(inv => inv.userId) }, subCategory, category, status: 'ACTIVE', ...(['volleyball', 'tennis', 'padel', 'pickleball'].includes(subCategory) && { approved: true }) },
                        select: { userId: true },
                    });
...
// zaten bir maçı/ilanı/hakemliği var mı — varsa çakışma bilgisini döner. Kullanıcı raporu:
// "aynı tarih/saatte hem tenis hem voleybol maçım oluyor" — MATCHED (kesinleşmiş) maçlarla
...
async function findSchedulingConflict(userId, matchDate, matchTime, duration, excludeId, opts = {}) {
    ...
    for (const cand of sameDay) {
        ...
        const isMine = cand.senderId === userId || cand.refereeId === userId
            ...
            || (Array.isArray(cand.unassignedPlayers) && cand.unassignedPlayers.some(p => p?.id === userId));
        if (!isMine) continue;
...
export const inviteToRival = async (req, res, next) => {
    try {
        ...
        if (isRefereeAd) {
            const refApprovalRequired = ['volleyball', 'tennis', 'padel', 'pickleball'].includes(rival.subCategory);
